...
Если вы не задали hostname в самом начале, прописываем:
...
Если настройки не получены, выполняйте настройку DHCP.
...
Перейдите в режим редактирования (нажмите `x` для удаления строки `BOOTPROTO=static`, затем `i` для вставки и замените на `BOOTPROTO=dhcp`). После внесения изменений нажмите `Ctrl+C` два раза и сохраните изменения командой:
...
2. Проверьте наличие каталогов для интерфейсов:
   ...
   Если каталогов меньше трёх (один из них создаётся по умолчанию), создайте необходимые:
...
## 9. Создание локальных учетных записей на HQ-RTR

### 9.1 Создание учетной записи `net_admin`
...
Вводим пароль: **P@$$word**

### 9.2 Настройка sudo без парольной аутентификации
...
### 9.4 Проверка работоспособности sudo
...
Если команда выполнена успешно (и строка изменена, например, на индикацию нового приглашения), значит настройка прошла корректно.

---

## 10. Создание локальных учетных записей на BR-RTR

<details>
<summary>(нажмите для развертывания)</summary>
  
### 10.1 Создание учетной записи `net_admin`
...
### 10.2 Настройка sudo без парольной аутентификации
...
### 10.4 Проверка работоспособности sudo
