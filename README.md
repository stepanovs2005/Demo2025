...
> - Присвоение имён устройствам,
> - Расчёт IP-адресации,
> - Настройку коммутации и маршрутизации.
> 
> В ходе проектирования и настройки сетевой инфраструктуры следует вести отчёт о своих действиях, включая таблицы и схемы, предусмотренные в задании. Итоговый отчёт должен содержать одну таблицу и пять отчётов о ходе работы. По окончании работы итоговый отчёт необходимо сохранить на диске рабочего места.
> 
...
>      - Пароль: P@$$word
>      - Пользователь должен обладать максимальными привилегиями и иметь возможность запускать sudo без дополнительной аутентификации.
> 
...
Если вы не задали hostname в самом начале, пропишите:
...
Проверьте, получили ли вы настройки по DHCP командой:

```bash
ip addr
```

Если настройки не получены, выполните настройку DHCP.
...
Перейдите в режим редактирования (удалите строку `BOOTPROTO=static`, затем нажмите `i` для вставки и замените на `BOOTPROTO=dhcp`). После внесения изменений нажмите `Ctrl+C` два раза и сохраните изменения командой:
...
2. Проверьте наличие каталогов для интерфейсов:
   ...
   Если каталогов меньше трёх (один из них создаётся по умолчанию), создайте необходимые каталоги для каждого интерфейса:

   ```bash
   mkdir /etc/net/ifaces/<название_интерфейса1> /etc/net/ifaces/<название_интерфейса2>
   ```

3. Отредактируйте файл настроек для каждого интерфейса командой:
...
8. Проверьте, появились ли IP-адреса у интерфейсов:
...
Удалите следующие строки (если присутствуют):
...
## 8. Настройка динамической маршрутизации (NAT) для офиса HQ
...
Пропишите IP-адрес с маской, соответствующий подсети (например, `192.168.10.1/26`).

### 8.1 Настройка на HQ-RTR
...
- `-A POSTROUTING` — правило применяется после маршрутизации.  
- `-o ens33` — правило для интерфейса, выходящего в Интернет.  
- `-s 192.168.10.0/26` — правило для трафика из указанной подсети.  
- `-j MASQUERADE` — маскарадинг.

### 8.2 Сохранение и автозапуск
...
Сохраните изменения (Ctrl+O, затем Ctrl+X) и перезагрузите сеть:
...
<details>
<summary>(нажмите для развертывания)</summary>

### 9.0 Настройка hostname
...
### 9.1 Настройка параметров сетевого интерфейса
...
## 10. Настройка динамической маршрутизации (NAT) для офиса BR
<details>
<summary>(нажмите для развертывания)</summary>
...
Пропишите IP-адрес с маской, соответствующий подсети (например, `192.168.30.1/27`).

### 10.1 Настройка на BR-RTR
...
- `-A POSTROUTING` — правило применяется после маршрутизации.  
- `-o ens33` — правило для интерфейса, выходящего в Интернет.  
- `-s 192.168.30.0/27` — правило для трафика из указанной подсети.  
- `-j MASQUERADE` — маскарадинг.

### 10.2 Сохранение и автозапуск
...
## 11. Создание локальных учетных записей на HQ-RTR

### 11.1 Создание учетной записи `net_admin`

```bash
useradd net_admin
```

Задайте пароль для `net_admin` командой:

```bash
passwd net_admin
```

Вводите пароль: **P@$$word**
...
и уберите `#` в начале. Затем сохраните изменения (Ctrl+O, затем Ctrl+X).

### 11.4 Проверка работоспособности sudo

Выйдите из root (команда `exit`) и войдите под пользователем `net_admin`:
...
Если команда выполнена успешно (например, приглашение изменилось), значит настройка прошла корректно.

---

## 12. Создание локальных учетных записей на BR-RTR

<details>
<summary>(нажмите для развертывания)</summary>
  
### 12.1 Создание учетной записи `net_admin`
...
### 12.2 Настройка sudo без парольной аутентификации
...
### 12.4 Проверка работоспособности sudo
...
Если команда выполнена успешно, настройка прошла корректно.
...
*Реализация настройки VLAN не выполнена.*

---

## 14. Настройка HQ-SRV

### 14.0 Настройка hostname
...
### 14.1 Настройка параметров сетевого интерфейса
...
Пропишите IP-адрес с маской, соответствующий подсети сервера (например, `192.168.10.2/26`).
...
## 15. Создание локальных учетных записей на HQ-SRV

### 15.1 Создание учетной записи `sshuser`

```bash
useradd -u 1010 sshuser
```

Задайте пароль для `sshuser` командой:

```bash
passwd sshuser
```

Вводите пароль: **P@ssw0rd**

### 15.2 Настройка sudo без парольной аутентификации
...
Уберите `#` в начале и сохраните изменения (Ctrl+O, затем Ctrl+X).

### 15.4 Проверка работоспособности sudo

Выйдите из root (команда `exit`) и войдите под пользователем `sshuser`:
...
### 16.1 Настройка параметров сетевого интерфейса
...
Пропишите IP-адрес с маской, соответствующий подсети сервера (например, `192.168.30.2/27`).
...
### 17.1 Создание учетной записи `sshuser`
...
### 17.2 Настройка sudo без парольной аутентификации
...
### 17.4 Проверка работоспособности sudo
