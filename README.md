...
- Сохраните изменения, нажав `Esc`, затем введите команду:
...
1. Создайте каталог для sub-интерфейса, заменив **<имя_физического_интерфейса>** на фактическое имя (например, `ens34`):

   ```bash
   mkdir /etc/net/ifaces/<имя_физического_интерфейса>.hq.100
   ```
...
3. Создайте файл для IP-адресации:

   ```bash
   nano /etc/net/ifaces/<имя_физического_интерфейса>.hq.100/ipv4address
   ```

   Введите IP-адрес в формате `ip/mask` (например, `192.168.10.2/26`).

#### 13.2 Создание VLAN для офиса HQ – VLAN200

1. Создайте каталог для sub-интерфейса:

   ```bash
   mkdir /etc/net/ifaces/<имя_физического_интерфейса>.hq.200
   ```
...
3. Создайте файл для IP-адресации:
   ...
   Введите IP-адрес в формате `ip/mask`.
...
1. Создайте каталог для sub-интерфейса:
   ...
   ```bash
   mkdir /etc/net/ifaces/<имя_физического_интерфейса>.hq.999
   ```
...
1. Создайте каталог для sub-интерфейса, заменив **<имя_физического_интерфейса>** (например, `ens33`):

   ```bash
   mkdir /etc/net/ifaces/<имя_физического_интерфейса>.br.999
   ```
...
2. **Настройка файла `options` для туннеля**
   ...
   Заходим в созданный каталог и создаём файл:
...
3. **Настройка IP-адреса туннеля**
   ...
   В той же папке создаём файл `ipv4address`:
...
8. **Настройка OSPF через vtysh**
   ...
   Внутри выполняем следующие команды:
...
2. **Настройка файла `options` для туннеля**
   ...
   Заходим в каталог и создаём файл:
